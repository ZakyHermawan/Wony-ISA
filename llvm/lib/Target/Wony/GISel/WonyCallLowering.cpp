...
#define DEBUG_TYPE "wony-call-lowering"

using namespace llvm;


WonyCallLowering::WonyCallLowering(const WonyTargetLowering &TLI)
    : CallLowering(&TLI) {}
...
struct WonyIncomingValueAssigner : public CallLowering::IncomingValueAssigner {
  WonyIncomingValueAssigner(CCAssignFn *AssignFn_, CCAssignFn *AssignFnVarArg_,
                             const WonySubtarget &Subtarget_, bool IsReturn)
...
struct WonyOutgoingValueAssigner : public CallLowering::OutgoingValueAssigner {
  WonyOutgoingValueAssigner(CCAssignFn *AssignFn_, CCAssignFn *AssignFnVarArg_)
...
bool WonyCallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
  ...
  auto MIB = MIRBuilder.buildInstrNoInsert(Wony::RET_PSEUDO);
  ...
  if (!FLI.CanLowerReturn) {
    ...
    const WonyTargetLowering &TLI = *getTLI<WonyTargetLowering>();
    ...
    WonyOutgoingValueAssigner Assigner(RetCC_Wony_Common, RetCC_Wony_Common);
...
bool WonyCallLowering::canLowerReturn(MachineFunction &MF,
  ...
  return !IsVarArg && checkReturn(CCInfo, Outs, RetCC_Wony_Common);
}

bool WonyCallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
                                             const Function &F,
...
bool WonyCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  WonyOutgoingValueAssigner Assigner(AssignFnFixed, AssignFnVarArg);
  ...
  MIRBuilder.buildInstr(Wony::ADJCALLSTACKUP)
