...
#define DEBUG_TYPE "wony-simple-cst-prop"
...
class WonySimpleConstantPropagation : public FunctionPass {
public:
  // Used for the pass registry.
  static char ID;

  WonySimpleConstantPropagation() : FunctionPass(ID) {
    // Technically we do not need to put this in the constructor.
    // Usually this call lives in the uber InitializeAllXXX.
    initializeWonySimpleConstantPropagationPass(
...
char WonySimpleConstantPropagation::ID = 0;

PreservedAnalyses
WonySimpleConstantPropagationNewPass::run(Function &F,
...
// function.
INITIALIZE_PASS(/*passImplementationName=*/WonySimpleConstantPropagation,
                /*commandLineArgName=*/DEBUG_TYPE,
                /*name=*/"Wony simple constant propagation",
                /*isCFGOnly=*/false,
                /*isAnalysis=*/false)

Pass *llvm::createWonySimpleConstantPropagationPassForLegacyPM() {
  return new WonySimpleConstantPropagation();
}
