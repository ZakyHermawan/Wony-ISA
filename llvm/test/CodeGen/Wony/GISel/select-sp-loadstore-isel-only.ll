...
; RUN: llc -mtriple wony--darwin -o - %s -global-isel=1 -global-isel-abort=1 -verify-machineinstrs -stop-after=finalize-isel | FileCheck %s
...
define i16 @alloca32SExtLoad(i32 %val) {
  ...
  ; CHECK-NEXT:   [[COPY1:%[0-9]+]]:gpr32 = COPY $d1
  ; CHECK-NEXT:   STRSP32 [[COPY1]], %stack.0.ptr, 0 :: (store (s32) into %ir.ptr)
  ; CHECK-NEXT:   [[LDRSEXTSP8_:%[0-9]+]]:gpr16 = LDRSEXTSP8 %stack.0.ptr, 0 :: (volatile dereferenceable load (s8) from %ir.ptr)
...
define i16 @alloca32ZExtLoad(i32 %val) {
  ...
  ; CHECK-NEXT:   [[COPY1:%[0-9]+]]:gpr32 = COPY $d1
  ; CHECK-NEXT:   STRSP32 [[COPY1]], %stack.0.ptr, 0 :: (store (s32) into %ir.ptr)
  ; CHECK-NEXT:   [[LDRZEXTSP8_:%[0-9]+]]:gpr16 = LDRZEXTSP8 %stack.0.ptr, 0 :: (volatile dereferenceable load (s8) from %ir.ptr)
